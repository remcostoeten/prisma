      ...
      <div className="relative">
        ...
        <div
          className={cn(
            ...
            {
              'scale-110 ring-1 ring-offset-1 ring-offset-background': isHovered,
        ...
        >
          ...
          <div className={cn(
            ...
            'animate-[ping_8s_ease-in-out_infinite] opacity-30',
            isAuthenticated ? 'bg-green-400/10' : 'bg-red-400/10'
          )} />
          <div className={cn(
            'absolute -inset-2 rounded-full',
            'animate-[ping_12s_ease-in-out_infinite] opacity-20',
          ...
          <div className={cn(
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="font-medium text-muted-foreground">Session Status</span>
                <span className={cn(
                  'px-1.5 py-0.5 rounded-full text-[10px] font-medium',
                  isAuthenticated ? 'bg-green-100/5 text-green-500/70' : 'bg-red-100/5 text-red-500/70'
                )}>
                  ...
                  <div className="text-muted-foreground/60">
                    {user.email && (
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-mono text-[10px] opacity-60">{user.email}</span>
                      </div>
                    )}
                    {user.name && (
                      <div className="flex justify-between items-center">
                        <span className="text-[10px]">Name:</span>
                        <span className="font-mono text-[10px] opacity-60">{user.name}</span>
                      </div>
                    )}
                    {user.provider && (
                      <div className="flex justify-between items-center">
                        <span className="text-[10px]">Provider:</span>
                        <span className="font-mono text-[10px] opacity-60">{user.provider}</span>
                      </div>
                    )}
                  </div>
                </div>
