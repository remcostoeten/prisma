...
interface DynamicSettingsContentProps {
  config: any;
  onBreadcrumbChange: (breadcrumbs: Array<{ href: string; label: string }>) => void;
}

export function DynamicSettingsContent({ config, onBreadcrumbChange }: DynamicSettingsContentProps) {
  ...
  const updateBreadcrumbs = (currentSection: string) => {
    const currentTab = allSettingsTabs.find(tab => tab.href === currentSection)
    if (currentTab) {
      onBreadcrumbChange([
