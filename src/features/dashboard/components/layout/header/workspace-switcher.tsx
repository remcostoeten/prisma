    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button className="group relative flex h-10 w-full items-center text-neutral-400 transition-all duration-300 ease-in-out hover:text-white cursor-pointer">
          ...
          >
            <span className="text-sm font-medium text-white">{selectedWorkspace.label}</span>
            <span className="text-xs text-neutral-500">Workspace</span>
          </div>
          <ChevronsUpDown className={cn("h-4 w-4 shrink-0 opacity-50 ml-auto", isCollapsed ? 'hidden' : 'block')} />
        </button>
      </PopoverTrigger>
      <PopoverContent className="w-[300px] p-0 b border border-[#1f1f1f] rounded-lg shadow-xl">
        <Command className="bg-transparent">
          ...
          />
          <CommandList>
            <CommandEmpty className="py-6 text-sm text-neutral-400">No workspace found.</CommandEmpty>
            <CommandGroup>
              {workspaces.map((workspace) => (
                ...
                >
                  ...
                  <div className="flex flex-col">
                    <span className="text-sm font-medium text-white">{workspace.label}</span>
                    <span className="text-xs text-neutral-500">Workspace</span>
                  </div>
